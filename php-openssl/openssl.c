...
PHP_FUNCTION(openssl_ecdh_compute_key);
...
ZEND_BEGIN_ARG_INFO(arginfo_openssl_ecdh_compute_key, 0)
	ZEND_ARG_INFO(0, pub_key)
	ZEND_ARG_INFO(0, dh_key)
ZEND_END_ARG_INFO()
...
/* for S/MIME handling */
	...
	PHP_FE(openssl_ecdh_compute_key,		arginfo_openssl_ecdh_compute_key)
...
PHP_FUNCTION(openssl_ecdh_compute_key)
{
	...
	outlen = (EC_GROUP_get_degree(group) + 7) / 8;
	point = EC_POINT_bn2point(group, pub, NULL, NULL);
	data = zend_string_alloc(outlen, 0);
	len = ECDH_compute_key((unsigned char*)ZSTR_VAL(data), outlen, point, ec, 0);
