...
int sharedstringlist_add_buffer (struct sharedstringlist* sharedstrings, const XML_Char* data, size_t datalen)
{
  ...
  if ((p = (XML_Char**)realloc(sharedstrings->strings, (sharedstrings->numstrings + 1) * sizeof(sharedstrings->strings[0]))) 
== NULL) {
    free(s);
    return 3;
  }
...
void shared_strings_callback_data_initialize (struct shared_strings_callback_data* data, struct sharedstringlist* 
sharedstrings)
{
  data->xmlparser = NULL;
...
void shared_strings_callback_find_shared_stringitem_start (void* callbackdata, const XML_Char* name, const XML_Char** atts)
{
  ...
  if (XML_Char_icmp(name, X("si")) == 0) {
    ...
    data->text = NULL;
    data->textlen = 0;
    XML_SetElementHandler(data->xmlparser, shared_strings_callback_find_shared_string_start, 
...
void shared_strings_callback_find_shared_stringitem_end (void* callbackdata, const XML_Char* name)
{
  ...
  if (XML_Char_icmp(name, X("si")) == 0) {
    ...
    data->text = NULL;
    data->textlen = 0;
    XML_SetElementHandler(data->xmlparser, shared_strings_callback_find_shared_stringitem_start, 
shared_strings_callback_find_sharedstringtable_end);
  } else {
